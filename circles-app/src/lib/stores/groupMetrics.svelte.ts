...
export async function fetchGroupMetrics(
  ...
  getMemberCount(circlesRpc, groupAddress, 'hour', '7 days').then(
    (r) => (target.memberCountPerHour = r)
  );
  getMemberCount(circlesRpc, groupAddress, 'day', '30 days').then(
    (r) => (target.memberCountPerDay = r)
  );
  getMintRedeem(circlesRpc, groupAddress, 'hour', '7 days').then(
    (r) => (target.mintRedeemPerHour = r)
  );
  getMintRedeem(circlesRpc, groupAddress, 'day', '30 days').then(
    (r) => (target.mintRedeemPerDay = r)
  );
  getWrapUnwrap(circlesRpc, groupAddress, 'hour', '7 days').then(
    (r) => (target.wrapUnwrapPerHour = r)
  );
  getWrapUnwrap(circlesRpc, groupAddress, 'day', '30 days').then(
    (r) => (target.wrapUnwrapPerDay = r)
  );
  getCollateralInTreasury(circlesRpc, groupAddress).then(
    (r) => (target.collateralInTreasury = r)
  );
  getGroupTokenHoldersBalance(circlesRpc, groupAddress).then(
    (r) => (target.tokenHolderBalance = r)
  );
